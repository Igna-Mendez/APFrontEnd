...
#                                                                            #
# This program was modified bt Johnny Hughes on 4/27/98 for sending e-mail   #
# via smtp sockets vice using UNIX send mail                                 #
##############################################################################
# Define Variables
#	 Detailed Information Found In README File.
# This script was modified to work with Windows NT by Johnny Hughes 4/27/98
# The smtp_server is the mail sever that this script uses to send messages 
$smtp_server = "smpt.oraculosemanal.com";
#
#
...
@referers = ('www.oraculosemanal.com');
...
sub send_mail {
#
# This section totally redone by Johnny Hughes 4/27/98
# to mail via SMTP sockets vice using UNIX sendmail
#
...
(!(socket(S,2,1,6))) && (&sockerror("Connect error! socket"));
(!(bind(S,$thisserver))) && (&sockerror("Connect error! bind"));
(!(connect(S,$remoteserver))) && (&sockerror("!! connection to $smtp_server has failed!"));
...
$DATA_IN = <S>;	
($DATA_IN !~ /^220/) && (&sockerror("data in Connect error - 220")); 

print S "HELO tg\n";
$DATA_IN = <S>;
($DATA_IN !~ /^250/) && (&sockerror("data in Connect error - 250")); 

print S "MAIL FROM:<$Config{'email'}>\n";
$DATA_IN = <S>;
($DATA_IN !~ /^250/) && (&sockerror("'From' address not valid")); 

print S "RCPT TO:<$Config{'recipient'}>\n";
$DATA_IN = <S>;
($DATA_IN !~ /^250/) && (&sockerror("'Recipient' address not valid")); 

print S "DATA\n";
$DATA_IN = <S>;
($DATA_IN !~ /^354/) && (&sockerror("Message send failed - 354")); 

    print S "To: $Config{'recipient'}\n";
    print S "From: $Config{'email'} ($Config{'realname'})\n";

    # Check for Message Subject
    if ($Config{'subject'}) { print S "Subject: $Config{'subject'}\n\n" }
    else                    { print S "Subject: WWW Form Submission\n\n" }

    print S "Below is the result of your feedback form.  It was submitted by\n";
    print S "$Config{'realname'} ($Config{'email'}) on $date\n";
    print S "----------------------------------------------------\n\n";

    if (@Print_Config) {
        foreach $print_config (@Print_Config) {
            if ($Config{$print_config}) {
                print S "$print_config: $Config{$print_config}\n\n";
            }
        }
    }
    ...
    # If a sort order is specified, sort the form fields based on that.      #
        ...
        foreach $sorted_field (@sorted_fields) {
            if ($Config{'print_blank_fields'} || $Form{$sorted_field} ||
                ...
                print S "$sorted_field: $Form{$sorted_field}\n\n";
            }
        }
    }
    ...
    foreach $env_report (@Env_Report) {
        if ($ENV{$env_report}) {
            print S "$env_report: $ENV{$env_report}\n";
        }
    }
print S ".\n";
$DATA_IN = <S>;
($DATA_IN !~ /^250/) && (&sockerror("Message send failed - try again - 250")); 
...
# added section for sockets errors by new e-mail routine
# added by Johnny Hughes 4/27/98
