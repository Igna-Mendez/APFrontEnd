...
for each numero in te0
	ubicacion = ubicacion + 1
	spUpNumero.parameters("@idNumero") = 0
	spUpNumero.parameters("@idSorteo") = idSorteo
	spUpNumero.parameters("@numero") = numero
	spUpNumero.parameters("@ubicacion") = ubicacion
	spUpNumero.Execute()
...
te1 = split(Request.Form("te1"), ",")  
for each numero in te1
	ubicacion = ubicacion + 1
    spUpNumero.parameters("@idNumero") = 0
	spUpNumero.parameters("@idSorteo") = idSorteo
	spUpNumero.parameters("@numero") = numero
...
if trim(Request.Form("teLetra")) <> "" then

set spUpLetra = Server.CreateObject("ADODB.Command")
spUpLetra.ActiveConnection = conn
spUpLetra.CommandText = "dbo.spUpLetra"
spUpLetra.CommandType = 4
spUpLetra.CommandTimeout = 0
spUpLetra.Prepared = true
spUpLetra.Parameters.Append spUpLetra.CreateParameter("@RETURN_VALUE", 3, 4)
spUpLetra.Parameters.Append spUpLetra.CreateParameter("@idLetra", 20, 3, 9, 0)
spUpLetra.Parameters.Append spUpLetra.CreateParameter("@idSorteo", 20, 1, 9, 0)
spUpLetra.Parameters.Append spUpLetra.CreateParameter("@ubicacion", 16, 1,1, 0)
spUpLetra.Parameters.Append spUpLetra.CreateParameter("@letra", 200, 1,10, 0)

ubicacion = 0
teLetra = split(Request.Form("teLetra"), ",")
for each letra in teLetra
	if trim(letra) <> "" then
	ubicacion = ubicacion + 1
    spUpLetra.parameters("@idLetra") = 0
	spUpLetra.parameters("@idSorteo") = idSorteo
	spUpLetra.parameters("@letra") = trim(letra)
	spUpLetra.parameters("@ubicacion") = ubicacion
	spUpLetra.Execute()
...
response.Redirect("./sendMail.asp?mails="+Request.Form("txtDestinatarios")+"&rte="+Request.Form("txtRemitente")+"&idSorteo="+cstr(idSorteo))
else
