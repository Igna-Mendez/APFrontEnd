...
if idSorteo = "" then
	if (Request.QueryString("idQuiniela") <> "") and (Request.QueryString("nroSorteo") <> "") then 
		...
		rsSorteo.Source = "select * from vsorteos_ordenado where idQuiniela = "+idQuiniela+" and nrosorteo = "+nroSorteo
...
if idSorteo = "" then
	if (Request.QueryString("idQuiniela") <> "") and (Request.QueryString("fechaSorteo") <> "") and (Request.QueryString("turno") <> "") then 
		...
		Set rsSorteo = Server.CreateObject("ADODB.Recordset")
		rsSorteo.ActiveConnection = MM_OraculoSemanal_STRING
		rsSorteo.Source = "select * from vsorteos_ordenado where idQuiniela = "+idQuiniela+" and turno = '"+turno +"' and datediff(day, fechaSorteo, '"+ fechaSorteo +"') = 0"
		rsSorteo.CursorType = 3
		rsSorteo.CursorLocation = 2
		rsSorteo.LockType = 1
		rsSorteo.Open()
...
if idSorteo = "" then
	...
	if (Request.QueryString("idQuiniela") <> "") and (fs <> "") then 
		...
		Set rsSorteo = Server.CreateObject("ADODB.Recordset")
		rsSorteo.ActiveConnection = MM_OraculoSemanal_STRING
		rsSorteo.Source = "select * from vsorteos_ordenado where idQuiniela = "+idQuiniela+" and datediff(day, fechaSorteo, '"+ fechaSorteo +"') = 0"
		rsSorteo.CursorType = 3
		rsSorteo.CursorLocation = 2
		rsSorteo.LockType = 1
		rsSorteo.Open()
...
<html>
<head>
<title>Documento sin t&iacute;tulo44</title>
